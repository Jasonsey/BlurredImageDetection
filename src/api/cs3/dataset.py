# 使用现有的清晰、模糊图片创建训练数据集
from PIL import Image
from pathlib import Path
from dataset2 import resize

grid_x = 30
grid_y = 30


def init_path(paths: list):
    for path in paths:
        p = Path(path)
        if not p.exists():
            p.mkdir(parents=True)


def load_images(ori_path1: Path):
    goods = []
    unclear = []
    bads = []
    paths = [ori_path1 / 'Good_License', ori_path1 / 'Bad_License', ori_path1 / 'Unclear_License']

    print('begin loading images')
    for path in paths:
        for image in path.glob('*.jpg'):
            if path.name == 'Good_License':
                goods.append(Image.open(image).convert('RGB'))
            elif path.name == 'Unclear_License':
                unclear.append(Image.open(image).convert('RGB'))
            else:
                bads.append(Image.open(image).convert('RGB'))
    print('finished loading images')
    return goods, bads, unclear


def focuse_image(img):
    w, h = 4, 3
    width, height = img.width, img.height
    w, h = (width // w, height // h) if width < height else (width // h, height // w)
    box = (w, h, width - w, height - h)
    return img.crop(box)


def crop_images(goods: list, bads: list, unclears: list, input_path: Path, clear_path: Path, blur_path: Path,
                unclear_path: Path):
    images = [goods, unclears, bads]
    for i in range(len(images)):
        for ii in range(len(images[i])):
            img = images[i][ii]
            img = focuse_image(img)
            img = resize(img)
            range_x = img.width // grid_x
            range_y = img.height // grid_y
            for x in range(range_x):
                for y in range(range_y):
                    bbox = (x * grid_x, y * grid_y, x * grid_x + grid_x, y * grid_y + grid_y)
                    slice_bit = img.crop(bbox)
                    if i == 0:
                        path1 = Path(input_path) / ('clear_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                        path2 = Path(clear_path) / ('clear_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                    elif i == 1:
                        path1 = Path(input_path) / ('unclear_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                        path2 = Path(unclear_path) / ('unclear_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                    else:
                        path1 = Path(input_path) / ('blur_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                        path2 = Path(blur_path) / ('blur_' + str(ii) + '_' + str(x) + '_' + str(y) + '.jpg')
                    slice_bit.save(path1, optimize=True)
                    slice_bit.save(path2, optimize=True)
            print(ii)
    print('All Done !')


def main():
    ori_path = "../../../data/input/License/Train"

    clear_path = "../../../data/output/cs542/train/clear/"
    unclear_path = "../../../data/output/cs542/train/unclear/"
    blur_path = "../../../data/output/cs542/train/blurred/"
    input_path = "../../../data/output/cs542/train/input_data/"

    init_path([clear_path, blur_path, input_path, unclear_path])

    good_img, bad_img, unclear_img = load_images(Path(ori_path))
    crop_images(good_img, bad_img, unclear_img, Path(input_path), Path(clear_path), Path(blur_path), Path(unclear_path))


def debug():
    ori_path = "../../../data/input/License/Train"
    good_img, bad_img, unclear_img = load_images(Path(ori_path))
    for img in bad_img:
        im = focuse_image(img)
        img.show()
        im.show()
        input('....')


if __name__ == '__main__':
    main()
    # debug()
